...
interface NormalizedCompressOptions {
  ...
  maxCanvasDimension: number;
  preserveLongImage: boolean;
  targetFileSizeKB: number | undefined;
  minQuality: number;
  maxIterations: number;
  concurrency: number;
  outputMode: CompressOutputMode;
  ...
  try {
    ...
    const result: ICompressImgResult = {
      ...
      ...(options.targetFileSizeKB === undefined
        ? {}
        : { targetAchieved: blob.size <= options.targetFileSizeKB * 1024 })
    };
 ...
 */
export function compressImage(file: File, options?: ICompressOptions): Promise<ICompressImgResult>;
export function compressImage(file: Blob, options?: ICompressOptions): Promise<ICompressImgResult>;
export function compressImage(file: FileList, options?: ICompressOptions): Promise<ICompressImgResult[]>;
