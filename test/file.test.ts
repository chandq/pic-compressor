...
import { compressImage, supportCanvas } from '../src';
...
function mockBitmap(width = 800, height = 600) {
  const close = vi.fn();
  globalThis.createImageBitmap = vi.fn(() => Promise.resolve({ width, height, close } as unknown as ImageBitmap));
  return { close };
}
...
function mockCanvas(output: Blob = new Blob(['compressed'], { type: 'image/jpeg' })) {
  ...
  const canvas = {
    ...
    toBlob: vi.fn((callback: TestBlobCallback) => callback(output)),
    toDataURL: vi.fn()
...
describe('file functions', () => {
  describe('supportCanvas', () => {
    ...
    it('returns false when creating the Canvas element throws', () => {
  ...
  describe('compressImage', () => {
    it('throws for invalid input and invalid options', () => {
      expect(() => compressImage('not a file' as never)).toThrow('not a file require be File or FileList');
      const file = createLargeFile();
      expect(() => compressImage(file, { quality: 2 })).toThrow('quality must be a finite number');
      expect(() => compressImage(file, { concurrency: Infinity })).toThrow('concurrency must be a finite number');
      expect(() => compressImage(file, { mime: 'image/gif' as never })).toThrow('Unsupported image mime type');
      expect(() => compressImage(file, { keepOriginalIfLarger: 'yes' as never })).toThrow(
        'keepOriginalIfLarger must be a boolean'
      );
      expect(() => compressImage(file, { onProgress: 100 as never })).toThrow('onProgress must be a function');
      expect(() => compressImage(file, { outputMode: 'full' as never })).toThrow(
        'outputMode must be "legacy" or "compact"'
      );
    });
    ...
    it('returns small files without requiring Canvas', async () => {
      ...
      await expect(compressImage(file)).resolves.toEqual({ file });
    });
    ...
    it('accepts Blob input for hybrid app and WebView integrations', async () => {
      ...
      const result = await compressImage(blob, { fileName: 'camera.jpg', outputMode: 'compact' });
    ...
    it('rejects when Canvas is unavailable', async () => {
      ...
      await expect(compressImage(createLargeFile())).rejects.toThrow('Current runtime environment not support Canvas');
    });
    ...
    it('uses async Blob encoding and compact mode without retaining large data URLs', async () => {
      ...
      const result = await compressImage(createLargeFile(), {
        quality: 0,
        maxSize: 1200,
        outputMode: 'compact'
      });
    ...
    it('falls back to HTMLImageElement decoding and releases its object URL', async () => {
      globalThis.createImageBitmap = vi.fn(() => Promise.reject(new Error('Bitmap decoding is unavailable')));
      ...
      expect(result).toMatchObject({ width: 640, height: 320, compressed: true });
      expect(createObjectURL).toHaveBeenCalledTimes(1);
      expect(revokeObjectURL).toHaveBeenCalledWith('blob:mock-image');
    });

    it('falls back to data URL encoding when canvas.toBlob is unavailable', async () => {
      mockBitmap(800, 600);
      const { canvas } = mockCanvas();
      Object.defineProperty(canvas, 'toBlob', { configurable: true, value: undefined });
      (canvas.toDataURL as Mock).mockReturnValue('data:image/webp;base64,Y29tcHJlc3NlZA==');
    ...
    it('uses a meaningful balanced default instead of deriving dimensions from file size', async () => {
      ...
      const smallSourceResult = await compressImage(createLargeFile('small-source.jpg', 'image/jpeg', 100), {
        outputMode: 'compact'
      });
      const largeSourceResult = await compressImage(createLargeFile('large-source.jpg', 'image/jpeg', 6000), {
        outputMode: 'compact'
      });
    ...
    it('preserves useful long-image dimensions while respecting the pixel budget', async () => {
      ...
      const result = await compressImage(createLargeFile('screenshot.png', 'image/png'), {
        mime: 'image/png',
        outputMode: 'compact'
      });
    ...
    it.each([
      ...
      async (_name, sourceWidth, sourceHeight, expectedWidth, expectedHeight) => {
        ...
        const { canvas } = mockCanvas();
        (canvas.toBlob as Mock).mockImplementation((callback: TestBlobCallback, type: string, quality: number) => {
          callback(new Blob([new Uint8Array(Math.round(quality * 700 * 1024))], { type }));
        });
        const input = createLargeFile('boundary.jpg', 'image/jpeg', 500);

        const result = await compressImage(input, { outputMode: 'compact' });
    ...
    it('reduces dimensions when an unchanged-size lossless long screenshot encodes larger', async () => {
      mockBitmap(386, 8192);
      const { canvas } = mockCanvas();
      (canvas.toBlob as Mock).mockImplementation((callback: TestBlobCallback, type: string) => {
      ...
      const result = await compressImage(input, { mime: 'image/png', outputMode: 'compact' });
    ...
    it('provides scenario presets and iterates toward the target file size', async () => {
      ...
      const result = await compressImage(createLargeFile('social.jpg', 'image/jpeg', 2000), {
        preset: 'social',
        outputMode: 'compact',
    ...
    it('reduces dimensions when quality alone cannot reach the target size', async () => {
      ...
      const result = await compressImage(createLargeFile('large.jpg', 'image/jpeg', 2000), {
        maxSize: 2000,
        quality: 0.8,
        minQuality: 0.8,
    ...
    it('applies long-image exemption to presets but not to explicit dimensions', async () => {
      ...
      const presetResult = await compressImage(createLargeFile(), {
        preset: 'social',
        targetFileSizeKB: null,
        outputMode: 'compact'
      });
      const explicitResult = await compressImage(createLargeFile(), {
        maxWidth: 1280,
        maxHeight: 1280,
    ...
    it('allows disabling a preset target size while retaining its dimensions', async () => {
      ...
      await compressImage(createLargeFile(), {
        preset: 'thumbnail',
        targetFileSizeKB: null,
        outputMode: 'compact'
      });
    ...
    it('does not skip a small source when a preset target is smaller than the source', async () => {
      ...
      const result = await compressImage(createLargeFile('avatar.jpg', 'image/jpeg', 40), {
        preset: 'thumbnail',
        outputMode: 'compact'
      });
    ...
    it('supports AVIF when available and can reject silent MIME fallback', async () => {
      ...
      const result = await compressImage(createLargeFile('photo.jpg'), {
        mime: 'image/avif',
        outputMode: 'compact'
      });
      ...
      (canvas.toBlob as Mock).mockImplementation((callback: TestBlobCallback) =>
        callback(new Blob(['fallback'], { type: 'image/png' }))
      );
      await expect(
        compressImage(createLargeFile(), { mime: 'image/avif', strictMime: true, outputMode: 'compact' })
    ...
    it('keeps the original when unchanged dimensions encode to a larger file', async () => {
      mockBitmap(800, 600);
      mockCanvas(new Blob([new Uint8Array(120 * 1024)], { type: 'image/jpeg' }));
      const input = createLargeFile('photo.jpg', 'image/jpeg', 100);
      ...
      expect(result).toEqual({ file: input });
    });
    ...
    it('limits output pixels and recognizes the MIME returned by Canvas', async () => {
      ...
      const result = await compressImage(createLargeFile('photo.jpeg'), {
        mime: 'image/webp',
        maxSize: 8000,
        maxPixels: 1_000_000,
        outputMode: 'compact'
    ...
    it('keeps legacy result fields by default', async () => {
      ...
      const result = await compressImage(origin);
    ...
    it('rejects encoding failures instead of leaving the promise pending', async () => {
      mockBitmap();
      const { canvas } = mockCanvas();
      (canvas.toBlob as Mock).mockImplementation((callback: TestBlobCallback) => callback(null));

      await expect(compressImage(createLargeFile(), { outputMode: 'compact' })).rejects.toThrow(
    ...
    it('propagates batch options and preserves input order', async () => {
      ...
      const results = await compressImage(createFileList(files), { minFileSizeKB: 40, concurrency: 1 });

      expect(results.map((result) => result.file)).toEqual(files);
      expect(createElement).not.toHaveBeenCalledWith('canvas');
    });

    it('aggregates progress across all files in a batch', async () => {
      const files = createFileList([
        createLargeFile('a.jpg', 'image/jpeg', 10),
        createLargeFile('b.jpg', 'image/jpeg', 20)
      ]);
      const onProgress = vi.fn();

      await compressImage(files, { minFileSizeKB: 30, concurrency: 1, onProgress });

      expect(onProgress.mock.calls.map(([progress]) => progress)).toEqual([0, 50, 50, 100]);
    });
    ...
    it('limits the number of images decoded concurrently', async () => {
      ...
      await compressImage(files, { concurrency: 2, outputMode: 'compact' });
    ...
    it('supports cancellation before work starts', () => {
      ...
      controller.abort();
      expect(() => compressImage(createLargeFile(), { signal: controller.signal })).toThrow(
        'Image compression aborted'
      );
    });
  });
});
